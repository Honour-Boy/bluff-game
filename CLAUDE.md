...
- **Spin overlay** (`useOnlinePlayerUiController`): each `spin_result` carries a monotonic `spinSeq` (orchestration) for a collision-free dedup key (the clinic bot's empty chamber repeats otherwise), and the overlay **self-clears** rather than depending on the `spinDismissed` flag (which is reset on every `spin_result` re-broadcast and raced the dismissal → the practice "spin hangs"). Applies to online mode too.
