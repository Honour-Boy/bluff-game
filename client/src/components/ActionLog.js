...
export function ActionLog({ lastAction }) {
  ...
  const messages = {
    ...
    bluff_called: ({ callerName, callerId }) =>
      callerName
    ...
    card_played: ({ playerName }) =>
      playerName
        ? `🃏 ${playerName} played a card face-down.`
        : "🃏 Card played face-down.",
    round_win: ({ winnerName }) =>
      `🏆 ${winnerName} won the round! All players redealt.`,
    game_over: ({ winnerName }) =>
      `🎉 GAME OVER — ${winnerName} is the last player standing!`,
    continued: ({ playerId }) => `→ Player continued their turn.`,
    disconnected: ({ playerName }) =>
      `🔌 ${playerName} disconnected — eliminated.`,
  };
