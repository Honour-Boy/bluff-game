      ...
      <svg
        ...
        style={{
          ...
          transition: animating
            ? "transform 8s cubic-bezier(0.1, 0, 0.15, 1)"
            : "none",
        }}
      >
        <circle
...
export function OnlinePlayerUI({
  ...
  const prevPlayerId =
    myTurnIdx >= 0 && turnOrder?.length > 1
      ? turnOrder[(myTurnIdx - 1 + turnOrder.length) % turnOrder.length]
      : null;
  const prevPlayer = prevPlayerId
    ...
    >
      ...
      {isLobby && (
        <div className="card" style={{ textAlign: "center" }}>
          ...
          >
            LOBBY — {alivePlayers.length} player
            {alivePlayers.length !== 1 ? "s" : ""} joined
          </div>
          {isHost ? (
            <>
              <button
      ...
      {!isEliminated && !isLobby && (
        ...
        >
          ...
          <div
            style={{ fontSize: 10, color: "var(--text-dim)", marginLeft: 2 }}
          >
            ...
            {!isFirstTurn && (
              <button
                ...
                style={{
                  ...
                  cursor:
                    bluffUsedThisTurn || cardPlayedThisTurn
                      ? "not-allowed"
                      : "pointer",
                }}
              >
      ...
      {isSpinPending && (
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            ...
            <div
              ...
              style={{
                ...
                background: lastAction.bluffCorrect
                  ? "rgba(74,255,128,0.04)"
                  : "rgba(255,74,110,0.04)",
              }}
            >
              <div
                style={{
              ...
              {lastAction.revealedCard ? (
                ...
                >
                  ...
                  >
                    <ShapeIcon
                      shape={lastAction.revealedCard.shape}
                      size={20}
                    />
                    <span
                      style={{
                    ...
                    >
                      {lastAction.revealedCard.shape === "whot"
                        ? "WHOT"
                        : lastAction.revealedCard.shape}
                    </span>
                    <span style={{ color: "var(--text-dim)", fontSize: 11 }}>
                      {lastAction.revealedCard.number}
                    </span>
                  </div>
                </div>
              ) : (
          ...
          {!isMySpinTurn && spinTargetPlayer && (
            ...
            >
              🔫 Waiting for <strong>{spinTargetPlayer.username}</strong> to
              pull the trigger...
            </div>
          )}
        </div>
      ...
      {isRoundEnd && (
        <div
          className="card"
          style={{ textAlign: "center", border: "1px solid var(--alive)" }}
        >
          <div
            style={{
              fontFamily: "'Bebas Neue', sans-serif",
          ...
          >
            {lastAction?.winnerId === myPlayer.id
              ? "🏆 You Won This Round!"
              : `🏆 ${lastAction?.winnerName ?? "?"} Won This Round!`}
          </div>
          {isHost && (
            <button
              className="primary"
        ...
        <div className="card">
            ...
            {alivePlayers.map((p) => (
              <button
                ...
                style={{
                  ...
                  background:
                    spectatingId === p.id
                      ? "rgba(232,255,74,0.06)"
                      : "var(--surface2)",
                  color:
                    spectatingId === p.id ? "var(--accent)" : "var(--text)",
                  borderRadius: "var(--radius)",
                  cursor: "pointer",
                }}
              >
          ...
          {spectatingId && spectatedHand.length > 0 && (
            <>
              ...
              >
                {alivePlayers.find((p) => p.id === spectatingId)?.username}
                &apos;s hand
              </div>
              <CardHand
                hand={spectatedHand}
        ...
        !isGameOver && (
          <div>
            ...
            <CardHand
              hand={myHand}
              selectedCardId={
                isMyTurn && isPlaying && !cardPlayedThisTurn
                  ? selectedCardId
                  : null
              }
      ...
      {pendingCard && (
        ...
        >
          ...
          >
            ...
            >
              {pendingCard.shape === "whot"
                ? "Whot (wild card)"
                : `${pendingCard.shape} ${pendingCard.number}`}
            </div>
