...
export function useGame(getAccessToken) {
  ...
  useEffect(() => {
    if (roomCode) {
      sessionStorage.setItem(
        "bluff_session",
        JSON.stringify({ roomCode, isHost, playerId }),
      );
    } else {
  ...
  useEffect(() => {
    ...
    const onBluffCalled = () =>
      notify("⚠️ Bluff called! Host: reveal the last card.", "warning");
    const onSpinAcknowledged = () => setSpinDismissed(true);
    const onHostDisconnecting = ({ countdown } = {}) => {
      notify(
        `Host disconnected. Game ends in ${countdown ?? 10}s if they don't return.`,
        "error",
      );
    };
  ...
  const resolveBluff = useCallback(
    (bluffIsCorrect) => {
  ...
  const declareRoundWin = useCallback(
    (winnerPlayerId) => {
      socket.emit(
        "round_win",
        { roomCode, playerId: winnerPlayerId },
        (res) => {
          if (!res.success) setError(res.error);
        },
      );
  ...
  const playCardOnline = useCallback(
    (cardId, nominatedShape) => {
      socket.emit(
        "play_card_online",
        { roomCode, playerId, cardId, nominatedShape },
        (res) => {
          if (!res.success) setError(res.error);
        },
      );
  ...
  const currentPlayer =
    roomState?.players?.find((p) => p.id === roomState?.currentPlayerId) ||
    null;
